...
def optimized_meal_detector(glucose_data):
    ...
    if len(glucose) >= 6:
        ...
        if avg_rate > 0.5:  # Further reduced threshold
            votes += 1
    ...
    if len(glucose) >= 12:
        ...
        for i in range(1, 12):
            if glucose[i] > glucose[i-1]:
                rise_points += 1
        if rise_points >= 5:  # Reduced threshold
            votes += 1
    
    # Rule 15: Quick rise detection
    if len(glucose) >= 3:
        quick_rise = glucose[2] - glucose[0]
        if quick_rise > 2:  # Reduced threshold
            votes += 1
    ...
    if len(glucose) >= 12:
        ...
        if 4 <= peak_idx <= 10:  # Peak between 20-50 minutes
            ...
            if peak_height > 8:  # Reduced threshold
                votes += 1
    
    # Rule 17: Any rise detection - Very sensitive
    if len(glucose) >= 4:
        any_rise = glucose[3] - glucose[0]
        if any_rise > 1:  # Extremely low threshold
            votes += 1
    
    # Rule 18: Baseline comparison - Very sensitive
    if len(glucose) >= 6:
        avg_after = np.mean(glucose[3:6])
        if avg_after > baseline + 1:  # Extremely low threshold
            votes += 1
    
    # Rule 19: Simple trend - Very sensitive
    if len(glucose) >= 5:
        trend = glucose[4] - glucose[0]
        if trend > 1:  # Extremely low threshold
            votes += 1
    
    # Rule 20: High glucose detection - Very sensitive
    if np.max(glucose) > baseline + 3:  # Extremely low threshold
        votes += 1
    
    # Rule 21: Any positive change - Extremely sensitive
    if len(glucose) >= 2:
        if glucose[1] > glucose[0]:
            votes += 1
    
    # Rule 22: Average increase - Extremely sensitive
    if len(glucose) >= 4:
    ...
    # Rule 23: Non-decreasing pattern - Extremely sensitive
    if len(glucose) >= 3:
        non_decreasing = True
        for i in range(1, 3):
            if glucose[i] < glucose[i-1]:
    ...
    # Rule 24: Above baseline - Extremely sensitive
    if len(glucose) >= 3:
        above_baseline = 0
        for i in range(1, 3):
    ...
    # Rule 25: Any glucose above 100 - Extremely sensitive
